...
window.CityHUD = (function () {
  ...
  const energyFillEl = document.getElementById('hud-energy-fill');
  const energyValueEl = document.getElementById('hud-energy-value');
  const energyRowEl = document.getElementById('hud-energy-row');
  const weaponRowEl = document.getElementById('hud-weapon-row');
  const powersMenuBtn = document.getElementById('btn-powers-menu');
  ...
  function setEnergy(v) {
    ...
    energyValueEl.textContent = Math.round(pct);
  }

  function setHudVisibility({ weapon, energy, powers }) {
    weaponRowEl.classList.toggle('hidden', !weapon);
    energyRowEl.classList.toggle('hidden', !energy);
    powersMenuBtn.classList.toggle('hidden', !powers);
  }
  ...
  return {
    setRole, setMoney, setHealth, setEnergy, setHudVisibility,
