// NPCs you can walk up to and talk to with E.
window.CityNPC = (function () {
  const NAMES = ['Yuki', 'Marco', 'Dana', 'Theo'];
  const LINES = {
    criminal: [
    ...
    villain: [
      "People cross the street when they see you coming. Good.",
      "Building an empire takes more than fear, you know.",
      "Careful — the heroes have been asking about you.",
    ],
  };
  ...
  function spawn(scene, spots) {
    const colors = [0xf97316, 0x14b8a6, 0xe11d48, 0x6366f1];
    return spots.map((pos, i) => {
      const mesh = buildNPCMesh(colors[i % colors.length]);
      mesh.position.copy(pos);
      mesh.rotation.y = Math.random() * Math.PI * 2;
      scene.add(mesh);
      return { mesh, name: NAMES[i % NAMES.length] };
    });
  }
  ...
  function lineFor(role) {
    const pool = LINES[role] || LINES.criminal;
